...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.chached:
            ...
            # --- CORRECTED DEPTH LOADING ---
            # Construct depth path with correct naming: image.jpg -> image_uint16.png
            depth_path = self.imlists[index].replace('/images', '/depths').replace('.jpg', '_uint16.png')
            
            # Load depth map (handling potential 16-bit images)
            ...
            if self.istrain:
                # Load the same depth map for large_depth
                large_depth_path = self.imlists[index].replace('/images', '/depths').replace('.jpg', '_uint16.png')
                
                large_depth = cv2.imread(large_depth_path, cv2.IMREAD_UNCHANGED)
                if large_depth is None:
                    raise FileNotFoundError(f"Depth file not found: {large_depth_path}")
                
                # Convert to grayscale if needed
                if len(large_depth.shape) == 3:
                    large_depth = cv2.cvtColor(large_depth, cv2.COLOR_BGR2GRAY)
                
                # Convert 16-bit to 8-bit if necessary
                if large_depth.dtype == np.uint16:
                    large_depth = (large_depth / 256).astype(np.uint8)
