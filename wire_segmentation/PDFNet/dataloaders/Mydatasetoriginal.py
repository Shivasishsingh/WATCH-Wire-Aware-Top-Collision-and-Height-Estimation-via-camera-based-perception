...
class MyDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.chached:
            ...
            if self.use_gt:
                # --- This block already handles .jpg to .png for masks ---
                gt_path = self.imlists[index].replace('/images','/masks').replace('.jpg','.png')
                if not os.path.exists(gt_path): # Handle if masks are also .jpg
                    gt_path = self.imlists[index].replace('/images','/masks')
            ...
            # --- MODIFIED BLOCK 1 START ---
            # Replaced the random logic to point to your depth_large folder
            # and corrected the file extension from .jpg to .png
            depth_path = self.imlists[index].replace('/images','/depth_large').replace('.jpg','.png')
            depth = cv2.cvtColor(cv2.imread(depth_path), cv2.COLOR_BGR2GRAY)
            # --- MODIFIED BLOCK 1 END ---

            depth = F.interpolate(torch.from_numpy(depth)[None,None,...],size=self.size,mode='bilinear',align_corners=True)[0]
            
            if self.istrain:
                # --- MODIFIED BLOCK 2 START ---
                # Pointed this to your depth_large folder and fixed extension
                large_depth_path = self.imlists[index].replace('/images','/depth_large').replace('.jpg','.png')
                large_depth = cv2.cvtColor(cv2.imread(large_depth_path), cv2.COLOR_BGR2GRAY)
                # --- MODIFIED BLOCK 2 END ---
