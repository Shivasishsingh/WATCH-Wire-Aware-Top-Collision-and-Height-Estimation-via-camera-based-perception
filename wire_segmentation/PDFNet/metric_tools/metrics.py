...
class IoUScore(object):
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        # Use adaptive threshold for binarization
        adaptive_threshold = _get_adaptive_threshold(pred, max_value=1)
        binary_pred = pred >= adaptive_threshold
        
        # Calculate IoU for foreground (wire class)
        iou = self.cal_iou(binary_pred, gt)
    ...
    def get_results(self) -> dict:
        iou = np.mean(np.array(self.ious, dtype=_TYPE))
        return dict(iou=iou)


class mIoU(object):
    """
    Mean Intersection over Union (mIoU) for binary segmentation.
    mIoU = (IoU_background + IoU_foreground) / 2
    
    For binary wire segmentation:
    - Foreground: Wire pixels
    - Background: Non-wire pixels
    """
    def __init__(self):
        self.miou_scores = []
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        miou = self.cal_miou(binary_pred, gt)
        self.miou_scores.append(miou)
        return miou

    def cal_miou(self, pred: np.ndarray, gt: np.ndarray) -> float:
        """
        Calculate mean IoU for binary segmentation.
        mIoU = (IoU_background + IoU_foreground) / 2
        
        TP (True Positive): pred=1, gt=1
        FP (False Positive): pred=1, gt=0
        FN (False Negative): pred=0, gt=1
        TN (True Negative): pred=0, gt=0
        """
        ...
        # IoU for foreground (wire class)
        iou_foreground = tp / (tp + fp + fn + _EPS)
        
        # IoU for background class
        iou_background = tn / (tn + fp + fn + _EPS)
    ...
    def get_results(self) -> dict:
        miou = np.mean(np.array(self.miou_scores, dtype=_TYPE))
        return dict(miou=miou)


class PrecisionRecallF1(object):
    """
    Precision, Recall, and F1-Score for binary segmentation.
    
    For binary wire segmentation:
    - Precision = TP / (TP + FP): Of predicted wires, how many are correct
    - Recall = TP / (TP + FN): Of actual wires, how many did we detect
    ...
    def step(self, pred: np.ndarray, gt: np.ndarray):
        ...
        precision, recall, f1 = self.cal_metrics(binary_pred, gt)
    ...
    def cal_metrics(self, pred: np.ndarray, gt: np.ndarray) -> tuple:
        ...
        # Precision: Of predicted wires, how many are correct
        precision = tp / (tp + fp + _EPS)
        
        # Recall: Of actual wires, how many did we detect
    ...
    def get_results(self) -> dict:
        precision = np.mean(np.array(self.precisions, dtype=_TYPE))
        recall = np.mean(np.array(self.recalls, dtype=_TYPE))
        f1 = np.mean(np.array(self.f1_scores, dtype=_TYPE))
